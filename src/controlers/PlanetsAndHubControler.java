...
public class PlanetsAndHubControler implements Serializable {
	...
	private PlanetsAndHubControler() {
		...
		// this.planetCount =
		// Integer.parseInt(DataControler._getInstance().getConfigProperty("planetCount"));
	...
	//
	// // this.mainHub.setLocation(new Point(this.random.nextInt(xOffSet) +
	// boardDeadBorder,
	// // this.random.nextInt(yOffSet) + boardDeadBorder));
	//
	// for (int i = 1; i <= this.planetCount; i++) {
	// final Point position = new Point(this.random.nextInt(xOffSet) +
	// boardDeadBorder,
	// this.random.nextInt(yOffSet) + boardDeadBorder);
